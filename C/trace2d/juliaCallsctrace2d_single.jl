...
xgrid = Vector{Float32}(undef,nx)   # grid x
ygrid = Vector{Float32}(undef,nx)   # grid y
ux = Vector{Float32}(undef,nx*ny)   # field x
uy = Vector{Float32}(undef,nx*ny)   # field y
xt = Vector{Float32}(undef,maxstep) # output x
yt = Vector{Float32}(undef,maxstep) # output y
ds = 1.0f0
...
npoints = ccall((:cEuler,"libtrace.so"),Cint,
   (Cint,Cint,Cint,Cfloat,Cfloat,Cfloat,Ptr{Cfloat},Ptr{Cfloat},
...
npoints = ccall((:cRk4,"libtrace.so"),Int32,
   (Int32,Int32,Int32,Float32,Float32,Float32,Ptr{Cfloat},Ptr{Cfloat},
   Ptr{Cfloat},Ptr{Cfloat},Ptr{Cfloat},Ptr{Cfloat}),
   nx, ny, maxstep, ds, 1.0, 10.0, xgrid, ygrid, ux, uy, xt, yt)
