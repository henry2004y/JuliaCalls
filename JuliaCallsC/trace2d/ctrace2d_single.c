...
/* Q10 = F[1,0], etc.                                 */
static float bilin_reg(float x, float y, float Q00,
			float Q01, float Q10, float Q11)
{
  float fout;
  //printf("Input = %.3f, %.3f\n",x, y);
  fout =
...
static void make_unit(int iSize, int jSize, float *ux, float *uy)
{
  int i;
  float magnitude;
 ...
 *float x, float y: X and Y of location to interp
 *float *field: Field to interpolate on
...
int cEuler(int iSize, int jSize,           /* Grid size and max steps */
	   int maxstep, float ds,         /* Maxsteps and step size  */
	   float xstart, float ystart,   /* Starting locations      */
	   float xGrid[], float yGrid[], /* Actual coord system     */
	   float *ux, float *uy,         /* Field to trace through  */
	   float x[], float y[])         /* x, y of result stream   */
{
...
#ifdef CTRACE2D_TEST
static int test_arrays(int iSize, int jSize, float xGrid[], float yGrid[],
		       float *ux, float *uy){
...
void test_check(float result, float answer)
{
  float diff, thresh;
...
{
  /* Declarations */
  float out=0, x=0.1, y=0.2, Q00=3.0, Q01=5.0, Q10=40.0, Q11=60.0;
  float sol1 = 7.460;
  int l[1];
  ...
  float xgrid[nx], ygrid[nx], *ux, *uy, xt[maxstep], yt[maxstep], ds=1.0;
  ux = malloc(nx * ny * sizeof(float));
  uy = malloc(nx * ny * sizeof(float));
